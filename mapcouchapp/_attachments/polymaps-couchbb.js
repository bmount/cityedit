...
po.geoJsonLabel = function(fetch) {
  var geoJsonLabel = po.layer(load, unload),
      container = geoJsonLabel.container(),
      url,
      clip = true,
      clipId = "org.polymaps." + po.id(),
      clipHref = "url(#" + clipId + ")",
  ...
  function load(tile, proj) {
    ...
    function update(data) {
      ...
      /* Convert the GeoJSON Labels to SVG textPath. */
      switch (data.type) {
        case "FeatureCollection": {
          for (var i = 0; i < data.features.length; i++) {
            var feature = data.features[i],
      ...
      tile.ready = true;
      updated.push.apply(tile.features, updated);
      geoJsonLabel.dispatch({type: "load", tile: tile, features: updated});
    }
  ...
  function move() {
    var zoom = geoJsonLabel.map().zoom(),
        tiles = geoJsonLabel.cache.locks(), // visible tiles
        key, // key in locks
        tile, // locks[key]
        features, // tile.features
        i, // current feature index
  ...
  geoJsonLabel.url = function(x) {
    if (!arguments.length) return url;
    url = typeof x == "string" && /{.}/.test(x) ? po.url(x) : x;
    if (url != null) features = null;
    if (typeof url == "string") geoJson.tile(false);
    return geoJsonLabel.reload();
  };

  geoJsonLabel.features = function(x) {
    if (!arguments.length) return features;
    if (features = x) {
      url = null;
      geoJsonLabel.tile(false);
    }
    return geoJsonLabel.reload();
  };

  geoJsonLabel.clip = function(x) {
    if (!arguments.length) return clip;
    if (clip) container.removeChild(clipPath);
    if (clip = x) container.insertBefore(clipPath, container.firstChild);
    var locks = geoJsonLabel.cache.locks();
    for (var key in locks) {
      if (clip) locks[key].element.setAttribute("clip-path", clipHref);
      else locks[key].element.removeAttribute("clip-path");
    }
  ...
  var __tile__ = geoJsonLabel.tile;
  geoJsonLabel.tile = function(x) {
    if (arguments.length && !x) geoJsonLabel.clip(x);
    return __tile__.apply(geoJsonLabel, arguments);
  };

  var __map__ = geoJsonLabel.map;
  geoJsonLabel.map = function(x) {
    if (x && clipRect) {
      var size = x.tileSize();
      clipRect.setAttribute("width", size.x);
      clipRect.setAttribute("height", size.y);
    }
    return __map__.apply(geoJsonLabel, arguments);
  };

  geoJsonLabel.scale = function(x) {
    if (!arguments.length) return scale;
    if (scale = x) geoJsonLabel.on("move", move);
    else geoJsonLabel.off("move", move);
    if (geoJsonLabel.map()) move();
    return geoJsonLabel;
  };

  geoJsonLabel.show = function(tile) {
    if (clip) tile.element.setAttribute("clip-path", clipHref);
    else tile.element.removeAttribute("clip-path");
    geoJsonLabel.dispatch({type: "show", tile: tile, features: tile.features});
    return geoJsonLabel;
  };

  geoJsonLabel.reshow = function() {
    var locks = geoJsonLabel.cache.locks();
    for (var key in locks) geoJsonLabel.show(locks[key]);
    return geoJsonLabel;
  };
